...
finalTestNames=""
echo "getting tests"
echo `curl --header "token: $apiKey" "$url/api/external/prioritized-tests/?project_name=$project&priority=$1&full_name=$fullname&test_suite_name=$testsuite&commit=$commitId" | sed 's/\"//g'`
